...
plotA_pm_min_x <- plotA_sq_perDay$Day_c[plotA_sq_perDay$n_sq==plotA_am_max]

plotA_pm_max <- max(plotA_sq_perDay$n_sq[plotA_sq_perDay$Shift=="PM"])
plotA_pm_max_x <- plotA_sq_perDay$Day_c[plotA_sq_perDay$n_sq==plotA_am_max]
...
# Combine AM + PM
plot_col <- cowplot::plot_grid(plotA_am_plot, plotA_pm_plot, ncol = 1)
...
plotB_plot <- plotB_df %>%
  ...
  geom_treemap_text(aes(label = tlabs), color = "grey15",
                    place = "centre") + 
  scale_fill_gradient(low = "thistle", high = "antiquewhite1") + 
  theme(panel.border = element_rect(colour = "grey15", fill=NA, size=1),
...
library(ggpubr)
plotABC <- ggarrange(plotA_plot,# Blank row
          ggarrange(plotB_plot, plotC_plot, ncol = 2), # Second row with box and dot plots
          nrow = 2                                      # Labels of the scatter plot
) + bgcolor("white") + theme(plot.margin = margin(2,2,2,2, "cm"))


ggsave("FinalSquirrelPlot.png", plot = plotABC,
